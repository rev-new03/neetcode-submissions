    ...
    public boolean isValidSudoku(char[][] board) {
        ...
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLS; j++) {
                ...
                HashSet<Character> rowValue = row.getOrDefault(i, new HashSet<>());//{1}
                if(rowValue.contains(num)){
                    return false;
                }else{
                    rowValue.add(num);
                    row.put(i,rowValue);//0:1,2
                }
                ...
                if(colValue.contains(num)){
                    ...
                    colValue.add(num);
                    col.put(j,colValue);//0:1,1:2
                }
                //checking for duplicates in squares
                String squareKey = String.valueOf(i/3)+ "," + String.valueOf(j/3); //0,0
                ...
                if(squareValue.contains(num)){
                    ...
                    squareValue.add(num);
                    squares.put(squareKey, squareValue);//0,0:1,2
                }
            }
        }
        return true;
